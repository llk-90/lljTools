...
function  request(){
    ...
    $.ajax(baseURL+"/weixiplusCommon/getRankdetails.webapp", {
        data:{
        ...
        success: function(data) {
            ...
            if (data.errorcode.code==0){
                ...
                //document.getElementById("signin").innerHTML=((data.userids[0].date) =="undefined"?"0":(data.userids[0].date));
                document.getElementById("signin").innerHTML=(data.userids[0].date)==undefined?"0":data.userids[0].date;
                document.getElementById("rank").innerHTML=data.userids[0].rank ==undefined?"0":(data.userids[0].rank);
                		...
                		for (var  i = 0;i < data.attences.length;i ++)
                		{
                			...
                			if(data.attences[i].rank == k){
                				if(data.attences[i].rank==1){
                					li.innerHTML = '<i><img src='+"../../img/geren/mylogo.jpg"+'></i> <h1>'+data.attences[i].stuName +'</h1> <span class='+"top1"+'>'+data.attences[i].rank +'</span> <p>报道时间  : '+ data.attences[i].date +'</p>';
                				}else if(data.attences[i].rank == 2){
                					li.innerHTML = '<i><img src='+"../../img/geren/mylogo.jpg"+'></i> <h1>'+data.attences[i].stuName +'</h1> <span class='+"top2"+'>'+data.attences[i].rank +'</span> <p>报道时间  : '+ data.attences[i].date +'</p>';
                				}else if(data.attences[i].rank == 3){
                					li.innerHTML = '<i><img src='+"../../img/geren/mylogo.jpg"+'></i> <h1>'+data.attences[i].stuName +'</h1> <span class='+"top3"+'>'+data.attences[i].rank +'</span> <p>报道时间  : '+ data.attences[i].date +'</p>';
                				}else {
                					li.innerHTML = '<i><img src='+"../../img/geren/mylogo.jpg"+'></i> <h1>'+data.attences[i].stuName +'</h1> <span>'+data.attences[i].rank +'</span> <p>报道时间  : '+ data.attences[i].date +'</p>  </li>';
                				}
